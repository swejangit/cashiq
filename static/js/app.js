...
async function switchView(view) {
    ...
    // Update Header Text
    const titles = {
        dashboard: { main: 'Dashboard', sub: 'Welcome back to Cashiq' },
        ...
        debts: { main: 'Debt Registry', sub: 'Track borrowed and lent pending funds' },
...
function renderDashboard() {
    ...
    // Debts summary (LENT vs BORROWED)
    const activeDebtsList = state.debts.filter(d => d.status === 'Active');
    const totalDebtsVal = activeDebtsList.reduce((sum, d) => {
        // Parse debt notes for type
        const isLent = d.notes && d.notes.startsWith('[LENT]');
        return isLent ? sum + d.pending_amount : sum - d.pending_amount;
    }, 0);
    ...
    let debtColorClass = "text-slate-500";
    let debtLabel = "Net Debts";
    if (totalDebtsVal > 0) {
        debtColorClass = "text-successGreen";
        debtLabel = "Net Receivable";
    } else if (totalDebtsVal < 0) {
        debtColorClass = "text-dangerRed";
        debtLabel = "Net Payable";
    }
            ...
            recentTxsHtml += `
                <tr class="text-sm group hover:bg-slate-50/60 transition duration-150">
                    <td class="py-3.5 px-1">
                        <p class="font-semibold text-slate-900 truncate max-w-[150px]">${t.note || 'Transaction'}</p>
...
function renderDebts() {
    ...
    // Calculate Summary numbers: Lent (Receivables) vs Borrowed (Payables)
    let totalLent = 0;
    let totalBorrowed = 0;
    
    state.debts.forEach(d => {
        if (d.status === 'Paid') return; // Exclude closed debts
        const isLent = d.notes && d.notes.startsWith('[LENT]');
        if (isLent) {
            totalLent += d.pending_amount;
        } else {
    ...
    let positionBg = 'from-slate-50';
    let positionText = 'Balanced position';

    if (netPosition > 0) {
        positionColor = 'text-successGreen';
        positionBg = 'from-emerald-50/50';
        positionText = 'Net Receivable (You are owed)';
    } else if (netPosition < 0) {
        positionColor = 'text-dangerRed';
        positionBg = 'from-rose-50/50';
        positionText = 'Net Payable (You owe money)';
    }
    ...
    if (state.debts.length === 0) {
        debtCardsHtml = `
            <div class="col-span-full bg-white border border-slate-200/60 rounded-2xl p-10 text-center flex flex-col items-center justify-center shadow-sm">
                ...
                <p class="font-bold text-slate-800 text-base">Debt Free!</p>
                <p class="text-xs text-slate-400 mt-1 mb-4">No borrowed or lent registry records exist.</p>
                <button onclick="openDebtModal()" class="px-4 py-2 bg-brand-600 hover:bg-brand-500 text-white text-xs font-bold rounded-lg transition shadow shadow-brand-500/10">
                    Add Debt Entry
                </button>
            </div>
        `;
        ...
        state.debts.forEach(d => {
            const isLent = d.notes && d.notes.startsWith('[LENT]');
            const displayNotes = d.notes ? d.notes.replace(/^\[LENT\]\s*|^\[BORROWED\]\s*/, '') : '';
            const isPaid = d.status === 'Paid';
            const isOverdue = d.status === 'Overdue';
            const paidPct = ((d.total_amount - d.pending_amount) / d.total_amount) * 100;
            ...
            debtCardsHtml += `
                <div class="bg-white border border-slate-200/60 rounded-2xl p-5 shadow-md shadow-slate-100/50 flex flex-col justify-between glow-card relative">
                    ...
                    <div class="mb-4">
                        <span class="text-[10px] uppercase font-extrabold tracking-wider ${isLent ? 'text-successGreen' : 'text-dangerRed'}">
                            ${isLent ? 'Lent to' : 'Borrowed from'}
                        </span>
                        <h4 class="text-base font-bold text-slate-900 mt-0.5">${d.person_name}</h4>
                        <p class="text-xs text-slate-400 mt-1 truncate" title="${displayNotes}">${displayNotes || 'No notes added.'}</p>
                    </div>
                    ...
                    <div class="space-y-3">
                        ...
                        <div class="w-full bg-slate-100 border border-slate-200/50 h-2 rounded-full overflow-hidden">
                            <div class="h-full bg-indigo-500 rounded-full transition-all duration-300" style="width: ${paidPct}%"></div>
                        </div>
                        ...
                        <div class="flex justify-between items-center text-[10px] text-slate-400">
                            ...
                            ${isPaid ? '' : `
                                <div class="flex items-center gap-2">
                                    <button onclick="payOffDebtPrompt(${d.id}, ${d.pending_amount}, '${d.person_name}', ${isLent})" class="text-xs text-brand-600 hover:text-brand-700 font-bold hover:underline transition-colors focus:outline-none">
                                        Pay Off
                                    </button>
                                    <span class="text-slate-200">|</span>
                                    <button onclick="editDebt(${d.id})" class="text-xs text-slate-400 hover:text-slate-800 transition-colors focus:outline-none">
                                        Edit
                                    </button>
                                </div>
                            `}
    ...
    container.innerHTML = `
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-6">
            <!-- lent total -->
            <div class="bg-white border border-slate-200/60 rounded-2xl p-5 shadow-md shadow-slate-100/50 relative overflow-hidden">
                <div class="flex justify-between items-start mb-2">
                    <span class="text-xs uppercase tracking-wider font-bold text-slate-400">Total Lent (Receivables)</span>
                    <span class="p-1 bg-emerald-500/10 text-emerald-600 rounded-lg"><i data-lucide="arrow-up-right" class="w-4 h-4"></i></span>
                </div>
                <h3 class="text-2xl font-extrabold text-emerald-600 tracking-tight font-outfit truncate">${formatCurrency(totalLent)}</h3>
            </div>
            <!-- borrowed total -->
            <div class="bg-white border border-slate-200/60 rounded-2xl p-5 shadow-md shadow-slate-100/50 relative overflow-hidden">
                <div class="flex justify-between items-start mb-2">
                    <span class="text-xs uppercase tracking-wider font-bold text-slate-400">Total Borrowed (Payables)</span>
                    <span class="p-1 bg-rose-500/10 text-rose-600 rounded-lg"><i data-lucide="arrow-down-left" class="w-4 h-4"></i></span>
                </div>
                <h3 class="text-2xl font-extrabold text-rose-600 tracking-tight font-outfit truncate">${formatCurrency(totalBorrowed)}</h3>
            </div>
            <!-- net balance -->
            <div class="col-span-1 md:col-span-2 bg-gradient-to-br ${positionBg} via-white to-white border border-slate-200/60 rounded-2xl p-5 shadow-md shadow-slate-100/50 relative overflow-hidden">
                <div class="flex justify-between items-start mb-2">
                    <span class="text-xs uppercase tracking-wider font-bold text-slate-400">Net Position</span>
                    <span class="p-1 bg-indigo-500/10 text-indigo-600 rounded-lg"><i data-lucide="activity" class="w-4 h-4"></i></span>
                </div>
                <h3 class="text-2xl font-extrabold ${positionColor} tracking-tight font-outfit truncate">${formatCurrency(netPosition)}</h3>
                <p class="text-[10px] text-slate-400 mt-1">${positionText}</p>
            </div>
        </div>

        <div class="flex items-center justify-between pt-4">
            <h4 class="text-xs font-bold uppercase tracking-wider text-slate-400">Debt Registry Cards</h4>
            <button onclick="openDebtModal()" class="px-4 py-2 bg-gradient-to-r from-brand-600 to-brand-500 hover:from-brand-500 hover:to-brand-400 text-white font-semibold rounded-lg text-xs transition flex items-center gap-1.5 shadow shadow-brand-500/10">
...
async function saveTransaction(e) {
    ...
    const payload = { type, amount, category_id, payment_method, transaction_date, note };
...
function openDebtModal(editDb = null) {
...
function editDebt(id) {
    const db = state.debts.find(d => d.id === id);
    if (db) {
        openDebtModal(db);
    }
}
...
async function saveDebt(e) {
    ...
    // Determine type from a confirmation or checkbox.
    const isLent = confirm(`Is this money LENT to ${person_name}? (Click OK if they owe you, Click Cancel if you owe them)`);
    const notes = isLent ? `[LENT] ${raw_notes}` : `[BORROWED] ${raw_notes}`;

    const payload = { person_name, total_amount, pending_amount, due_date, status, notes };
    const method = id ? 'PUT' : 'POST';
